...
class Email(models.Model):
    ...
    body = RichTextField()
    attachement = models.FileField(upload_to="email_attachements/", blank=True)
    ...
    def open_rate(self):
        ...
        opened_count = EmailTracking.objects.filter(
            email=self, opened_at__isnull=False
        ).count()
    ...
    def click_rate(self):
        ...
        if opened_count > 0:
            clicked_count = EmailTracking.objects.filter(
                email=self, clicked_at__isnull=False
            ).count()
...
class Sent(models.Model):
    ...
    def __str__(self):
        return f"{self.email} - {self.total_sent} emails_sent"
...
class EmailTracking(models.Model):
    ...
    def __str__(self):
        return self.email.subject  # type: ignore
